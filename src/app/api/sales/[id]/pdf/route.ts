...
export async function GET(
  ...
  try {
    ...
    const [order] = await odoo.searchRead<SaleOrder>('sale.order', [['id', '=', orderId]], {
      fields: ['name', 'partner_id', 'date_order', 'commitment_date', 'amount_total', 'amount_untaxed', 'amount_tax', 'state', 'note'],
    });
...
<head>
  <meta charset="utf-8">
  <title>見積書/受注書 - ${order.name}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .totals-row.grand { font-size: 16px; font-weight: bold; border-bottom: 2px solid #2563eb; color: #2563eb; }
...
<body>
  <div class="document">
    <div class="header">
      <div>
        <div class="title">${order.state === 'draft' || order.state === 'sent' ? '見積書' : '受注書'}</div>
    ...
    <div class="parties">
      <div class="party">
        <div class="party-label">お客様 / Customer</div>
        <div class="party-name">${partner?.name || (Array.isArray(order.partner_id) ? order.partner_id[1] : '-')} 様</div>
    ...
    <div class="meta">
      <div class="meta-item">
        <div class="meta-label">注文日</div>
        <div class="meta-value">${formatDate(order.date_order)}</div>
      </div>
      ${order.commitment_date ? `
      <div class="meta-item">
        <div class="meta-label">納品予定日</div>
        <div class="meta-value">${formatDate(order.commitment_date)}</div>
      </div>
      ` : ''}
      <div class="meta-item">
      ...
      <tbody>
        ${lines.map((line, idx) => `
          <tr>
            ...
            <td class="text-center">${line.product_uom_qty} ${Array.isArray(line.product_uom) ? line.product_uom[1] : ''}</td>
    ...
    return NextResponse.json({
      ...
      data: {
        html: pdfHtml,
        filename: `SO_${order.name.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
        order: {
          name: order.name,
