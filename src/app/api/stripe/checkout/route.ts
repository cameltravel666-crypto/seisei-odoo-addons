...
export async function POST(request: NextRequest) {
  try {
    ...
    const productRecords = await prisma.subscriptionProduct.findMany({
      where: {
        productCode: { in: products.map((p: { productCode: string }) => p.productCode) },
        isActive: true,
      },
    });
    ...
    for (const p of products as Array<{ productCode: string; quantity?: number }>) {
      const product = productRecords.find((pr) => pr.productCode === p.productCode);
      if (!product) {
        return NextResponse.json(
          { success: false, error: { code: 'INVALID_PRODUCT', message: `Product not found: ${p.productCode}` } },
          { status: 400 }
        );
      }

      const priceId = billingCycle === 'yearly' ? product.stripePriceYearly : product.stripePriceMonthly;
      if (!priceId) {
        return NextResponse.json(
          { success: false, error: { code: 'STRIPE_NOT_CONFIGURED', message: `Stripe price not configured for: ${p.productCode}` } },
          { status: 400 }
        );
      }
    ...
    const hasBasePlan = productRecords.some((p) => p.productType === 'BASE_PLAN');
    if (!hasBasePlan) {
      return NextResponse.json(
    ...
    const billingEmail = user.tenant.billingEmail || `${user.tenant.tenantCode}@seisei.tokyo`;
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (checkoutSession.metadata?.tenantId !== session.tenantId) {
      return NextResponse.json(
        { success: false, error: { code: 'FORBIDDEN', message: 'Session does not belong to this tenant' } },
        { status: 403 }
      );
    }
