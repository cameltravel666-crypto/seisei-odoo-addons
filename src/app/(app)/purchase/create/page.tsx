...
import { ArrowLeft, Plus, Minus, Search, Check, ShoppingCart, Package } from 'lucide-react';
  ...
  return (
    <div className="pb-32 md:pb-20">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <h1 className="text-lg font-semibold text-gray-900">{t('purchase.createOrder')}</h1>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 mb-3">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100 bg-gray-50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-700">{t('purchase.orderLines')}</span>
          </div>
          {orderLines.length > 0 && (
        ...
        {orderLines.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            <div className="text-sm font-medium">{t('purchase.noProducts')}</div>
            <div className="text-xs mt-1 text-gray-400">{t('purchase.addProduct')}</div>
          </div>
        ) : (
                    ...
                    <div className="flex justify-between items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-gray-900 text-base leading-snug line-clamp-2">
                          {line.productName || t('purchase.noProducts')}
                        </div>
                        <div className="flex items-center gap-3 mt-1.5 text-sm text-gray-500">
                          <span className="inline-flex items-center gap-1">
                            <span className="text-gray-400">{t('purchase.quantity')}:</span>
      ...
      <div className="fixed bottom-16 md:bottom-0 left-0 right-0 md:left-64 bg-white border-t border-gray-200 shadow-lg z-40">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex flex-col items-end gap-1.5">
            {!selectedSupplier && (
              <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded">{t('purchase.selectSupplierFirst')}</span>
            )}
            {selectedSupplier && orderLines.length === 0 && (
              <span className="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded">{t('purchase.addProductsFirst')}</span>
            )}
            <button
