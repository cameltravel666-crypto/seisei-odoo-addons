...
# 3. Check if stack directory exists (for first deployment)
if [ ! -e "$STACK_DIR" ]; then
    log_warn "Stack directory does not exist yet (first deployment)"
    log_info "Skipping compose file checks - will be created during deployment"
else
    # 3a. Check compose file exists
