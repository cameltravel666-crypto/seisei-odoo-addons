#!/usr/bin/env node
/**
 * Stripe Product Sync Script (CommonJS version)
 * Run this script to sync all subscription products to Stripe
 *
 * Usage: node scripts/stripe-sync.cjs
 */
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        // Create monthly price (JPY doesn't use cents, use actual amount)
        const monthlyPrice = await stripe.prices.create({
          product: stripeProduct.id,
          unit_amount: Math.round(Number(product.priceMonthly)),
          currency: 'jpy',
          recurring: {
            interval: 'month',
          },
