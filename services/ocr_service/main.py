...
1. "小計" / "税抜合計" / "税抜" → net
2. "外税額" / "外税額　8%" / "外税額　10%" → 各税率の税額
3. "合計" / "お会計" / "総合計" → gross
4. "8%対象" / "8%対象額" → r8_gross (あれば)
5. "10%対象" / "10%対象額" → r10_gross (あれば)

【STEP 2: 外税 or 内税を判定】:
A) 「外税額」の表記がある → 外税形式
   → r8_gross = 0, r10_gross = 0 に強制設定
   → r8_tax と r10_tax のみ「外税額　X%」行から読取

B) 「8%対象」「10%対象」の表記がある → 内税形式
   → r8_gross, r10_gross を「X%対象」行から読取
   → r8_tax, r10_tax を「内消費税X%」行から読取

C) どちらでもない → net + tax = gross を検証して判定

【外税レシートの例（r8_gross/r10_gross は必ず0）】:
```
小計            ¥7,415  → net = 7415
...
【内税レシートの例（r8_gross/r10_gross に値あり）】:
```
