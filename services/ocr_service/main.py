...
【通帳の列構成を正しく認識すること】
日本の通帳は通常以下の列で構成されます：
  年月日 | 記号(D/W/C) | お支払金額(出金) | お預り金額(入金) | 差引残高 | 備考
「差引残高」と「備考」は別の列です。絶対に結合しないでください。
備考欄には店番号(例:983,980,441)やコード(CFS等)が記載されています。
残高の前に*マークがある場合がありますが、*は無視して数値のみ抽出してください。
...
出力フォーマット（JSON only, no markdown）:
{
  ...
  "balance_start": 期首残高（最初の取引前の残高、数値）,
  ...
  "transactions": [
    {
      ...
      "deposit": 入金額（数値、0なら0）,
      "balance": 取引後の差引残高（数値。備考欄の数字を含めないこと）,
      "reference": "備考欄の内容（店番号やコード、なければ空文字）"
    }
  ]
}

【重要な注意事項】
- ⚠️ 差引残高と備考は別々の列。例: 残高=211,668 備考=983 → balance=211668, reference="983"
  絶対に 211668983 のように結合しないこと！
- ⚠️ 日付は通帳に印字された形式のまま出力すること（自分で西暦変換しない）
  例: 通帳に「7-12-8」と印字 → "date": "7-12-8"（そのまま出力）
  ※ 令和→西暦の変換はシステム側で自動処理するため、AIが変換する必要はありません
- 金額のカンマは除去して数値で返す
- 記号: D=入金(deposit), W=出金(withdrawal), C=小切手等の入金
