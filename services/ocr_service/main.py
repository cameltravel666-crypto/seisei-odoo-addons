...
【最重要】必ず下記のキーワード行から数字を読み取る（商品明細を足し算しない）:
1. "小計" / "税抜合計" / "税抜" → net (税前金額)
2. "外税額" / "消費税" / "内消費税" → tax (消費税合計)
3. "合計" / "お会計" / "総合計" → gross (最終支払額)
4. "外税額　8%" / "消費税8%" / "8%対象" → r8_tax
5. "外税額　10%" / "消費税10%" / "10%対象" → r10_tax

【税込/税抜の判定】:
- 「外税」「税別」表記あり → 小計=税抜, 合計=小計+税
- 「内税」「税込」表記あり → 小計=税込
- 判別できない場合 → net + tax が gross と一致するか検証

【8%/10%対象額の読み方】:
パターンA（外税・税別レシート）:
- "外税額　8%  ¥592" → r8_tax = 592, r8_gross = 0 (小計に含まれる)
- "外税額　10% ¥1" → r10_tax = 1, r10_gross = 0

パターンB（内税レシート with 税率別内訳）:
