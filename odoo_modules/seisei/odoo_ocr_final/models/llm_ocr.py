...
BANK_STATEMENT_OCR_PROMPT = '''あなたは日本の銀行通帳（普通預金通帳）の読み取り専門AIです。
...
- 各行に金額は1つだけ: 出金欄に★数値 → withdrawal、入金欄に★数値 → deposit
...
例1: 為替手数料   | ★220   | フリコミ テスウリヨウ |             | RT
     ...
     （★220=出金額、フリコミ テスウリヨウ=テキスト→descriptionに結合）

例2: 振込IB2      |        | ★317,360            |             | RT
                  |        | フカヤ モエカ         |             |
     → description="振込IB2 フカヤ モエカ", withdrawal=0, deposit=317360
