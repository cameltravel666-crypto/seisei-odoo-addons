...
(function () {
  ...
  const i18n = {
    ...
    en_US: {
      ...
      pay_instruction:
        "Please show the information below at the counter to pay.",
      pay_table: "Table",
      pay_order: "Order ID",
  ...
  // ==================== DOM Elements ====================
  let $app, $categories, $products, $content, $cartBadge, $cartAmount;
  ...
  function init() {
    ...
    console.log(
      "QR Ordering initialized successfully. Build:",
      window.QR_ORDERING_BUILD || "unknown",
    );
    ...
    const diagOverflow = {
      ...
      scrollLockClass:
        document.documentElement.classList.contains("qr-scroll-locked"),
    };
    console.log("[QR Ordering] Scroll Diagnostic:", diagOverflow);

    // 验收检查：html/body 默认不应该是 hidden
    if (
      diagOverflow.htmlComputedOverflow === "hidden" ||
      diagOverflow.bodyComputedOverflow === "hidden"
    ) {
      console.warn(
        "[QR Ordering] ⚠️ WARNING: html/body has overflow:hidden at init!",
        diagOverflow,
      );
    }
  ...
  function checkRootVisibility(timing) {
    ...
    if (!$app) {
      console.error(
        `[QR Ordering] [${timing}] Root element #qr-ordering-app not found!`,
      );
    ...
    if (rect.height === 0 || rect.width === 0) {
      ...
      // ========== 诊断 0x0 的元凶 ==========
      console.error(
        `[QR Ordering] [${timing}] 🔍 DIAGNOSIS: Root is 0x0, investigating...`,
      );
      ...
      while (parent && level <= 6) {
        ...
        if (parentStyle.visibility === "hidden")
          parentIssues.push("visibility:hidden");
        if (parentRect.height === 0)
          parentIssues.push(`height:0 (${Math.round(parentRect.height)}px)`);
        if (parentRect.width === 0)
          parentIssues.push(`width:0 (${Math.round(parentRect.width)}px)`);

        if (parentIssues.length > 0 || level === 1) {
          console.error(
            `[QR Ordering] Parent #${level} (${parent.tagName}${parent.id ? "#" + parent.id : ""}${parent.className ? "." + parent.className.split(" ")[0] : ""}):`,
            {
        ...
        if (parentIssues.length > 0) {
          console.error(
            `[QR Ordering] 🎯 FOUND CULPRIT: Parent #${level} has issues:`,
            parentIssues,
          );
          break;
        }
    ...
    if (issues.length > 0) {
      console.error(
        `[QR Ordering] [${timing}] Root visibility check failed:`,
        issues.join(", "),
      );
      // 3s 检查失败时才显示错误覆盖层
      if (timing === "3s") {
        showFatalError("页面内容不可见，可能被 CSS 隐藏或内容未加载", null);
      }
    } else {
      console.log(
        `[QR Ordering] [${timing}] Root visibility check passed: children=${childCount}, rect=${Math.round(rect.width)}x${Math.round(rect.height)}`,
      );
    }
  }
  ...
  function setupDomObserver($app) {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (previousChildCount > 0 && currentChildCount === 0) {
        console.error(
          "[QR Ordering] DOM cleared detected! previousChildren:",
          previousChildCount,
          "-> currentChildren:",
          currentChildCount,
        );
    ...
    const parentObserver = new MutationObserver((mutations) => {
      if (!document.getElementById("qr-ordering-app")) {
        ...
        showFatalError("页面根节点被移除，可能是脚本冲突", null);
      }
    });
  ...
  function showDebugBadge($app) {
    ...
    setInterval(() => {
      ...
      const rectNow = $appNow
        ? $appNow.getBoundingClientRect()
        : { width: 0, height: 0 };
  ...
  function setupEventListeners() {
    ...
    // Search
    document
      .getElementById("qr-search-input")
    ...
    // 主按钮（提交订单/追加下单/去前台支付）
    document
      .getElementById("qr-primary-btn")
      ?.addEventListener("click", handlePrimaryBtnClick);

    // 次按钮（查看购物车/查看订单）
    document
      .getElementById("qr-secondary-btn")
      ?.addEventListener("click", handleSecondaryBtnClick);

    // 购物车图标点击
    document
      .getElementById("qr-cart-icon-btn")
      ?.addEventListener("click", openCartModal);

    // 支付弹窗关闭
    document
      .getElementById("qr-pay-close")
      ?.addEventListener("click", closePayModal);
    document
      .getElementById("qr-pay-done")
      ?.addEventListener("click", closePayModal);

    // 复制按钮
    document
      .getElementById("qr-copy-table")
      ?.addEventListener("click", () =>
        copyToClipboard(state.tableName, t("copy_success")),
      );
    ...
    document
      .getElementById("qr-order-status-badge")
      ?.addEventListener("click", handleStatusChipClick);

    // ========== 订单状态 toast 事件 ==========
    document
      .getElementById("qr-order-toast-close")
      ?.addEventListener("click", hideOrderStatusToast);
    document
      .getElementById("qr-order-toast-copy-ref")
      ?.addEventListener("click", () => {
    ...
    // 兼容旧按钮 ID（如果存在）
    document
      .getElementById("qr-view-cart-btn")
      ?.addEventListener("click", openCartModal);
    document
      .getElementById("qr-view-orders-btn")
      ?.addEventListener("click", openOrderModal);
    document
      .getElementById("qr-checkout-btn")
      ?.addEventListener("click", handleCheckout);

    // Cart modal close
    document
      .getElementById("qr-cart-close")
    ...
    // Order modal close
    document
      .getElementById("qr-order-close")
      ?.addEventListener("click", closeOrderModal);

    // Submit order
    document
      .getElementById("qr-submit-order-btn")
      ?.addEventListener("click", submitOrder);

    // Add more (from order modal)
    document
      .getElementById("qr-add-more-btn")
      ?.addEventListener("click", () => {
        closeOrderModal();
      });
  ...
  async function loadInitData() {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        const cartOrder = state.orders
          ? state.orders.find((o) => o.state === "cart")
          : null;
        ...
        console.log(
          `QR Ordering loaded successfully. Build: ${window.QR_ORDERING_BUILD || "unknown"}`,
        );
      } else {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        try {
          ...
          // UI 更新错误不影响订单提交成功
          console.error(
            "[submitOrder] UI update error (order was submitted):",
            uiError,
          );
        ...
        // API 返回失败
        const errMsg =
          result && result.message ? result.message : t("order_failed");
  ...
  function renderProducts(filter = "") {
    ...
    if (filter) {
      ...
      products = products.filter((p) => {
        ...
        const keywordMatch =
          p.search_keywords &&
          p.search_keywords.toLowerCase().includes(lowerFilter);
  ...
  function renderOrders() {
    ...
    const totalUntaxed =
      footerState.totalOrderAmountUntaxed || totalIncl - totalTax;

    // 渲染商品明细（显示名称、数量、金额）
    const linesHtml = allLines
      .map(
        (l) => `
  ...
  function updateCartUI() {
    ...
    if ($cartAmount)
      $cartAmount.textContent = `${t("currency")}${totalAmount.toFixed(0)}`;
  ...
  function openPayModal() {
    ...
    if ($paySubtotal)
      $paySubtotal.textContent = `${t("currency")}${subtotal.toFixed(0)}`;
    if ($payTax)
      $payTax.textContent = `${t("currency")}${taxAmount.toFixed(0)}`;
    if ($payAmount)
      $payAmount.textContent = `${t("currency")}${totalInclTax.toFixed(0)}`;
  ...
  function showOrderStatusToast(info = {}) {
    ...
    const canAdd =
      info.canAdd !== undefined ? info.canAdd : footerState.state === "D";
  ...
  const ScrollLock = {
    ...
    lock(reason = "default") {
      ...
      if (prevCount === 0) {
        ...
        console.log(
          `[ScrollLock] Locked (reason: ${reason}, count: ${this._lockCount})`,
        );
      } else {
        console.log(
          `[ScrollLock] Already locked, added reason: ${reason} (count: ${this._lockCount})`,
        );
      }
    },
    ...
    unlock(reason = "default") {
      ...
      if (this._lockCount === 0) {
        ...
        console.log(
          `[ScrollLock] Still locked (remaining: ${this._lockCount}, reasons: ${Array.from(this._lockReasons.keys()).join(", ")})`,
        );
      }
    },
  ...
  function openTemplateModal(templateId) {
    ...
    const variantListHtml = variants
      .map(
        (v) => `
  ...
  function updateDebugPanel() {
    ...
    if (assetsEl) {
      assetsEl.textContent =
        typeof odoo !== "undefined" ? "✓ loaded" : "✗ failed";
    }

    // Update JS status
    const jsEl = document.getElementById("qr-debug-js");
    if (jsEl) {
      jsEl.textContent = window.QR_ORDERING_BUILD
        ? "✓ " + window.QR_ORDERING_BUILD
        : "✗ failed";
    }
  }
  ...
  window.QrOrdering = {
    ...
    selectVariant(variantId) {
      ...
      const variant = (state.selectedTemplate.variants || []).find(
        (v) => v.id === variantId,
      );
      if (!variant) return;
      ...
      if (nameEl)
        nameEl.textContent = variant.variant_display_name || variant.name;
      const priceEl = document.getElementById("qr-detail-price");
      if (priceEl)
        priceEl.textContent = `${t("currency")}${(variant.price_with_tax || variant.price || 0).toFixed(0)}`;
    ...
    addFromDetail() {
      ...
      const qty = parseInt(
        document.getElementById("qr-detail-qty").textContent,
      );
  ...
  try {
    ...
    if ($app) {
      $app.innerHTML =
        '<div style="padding: 20px; text-align: center;"><p>页面加载失败，请刷新重试</p><p style="color: #999; font-size: 12px;">' +
        error.message +
        "</p></div>";
    }
  }
